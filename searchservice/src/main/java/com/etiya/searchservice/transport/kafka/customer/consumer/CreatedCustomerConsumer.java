...
public class CreatedCustomerConsumer {
    ...
    public Consumer<CreateCustomerEvent> customerCreated(){
        return event -> {
            CustomerSearch customerSearch = new CustomerSearch(event.customerId(), event.customerNumber(), event.firstName(), event.middleName(), event.lastName(), event.nationalId(), event.motherName(), event.fatherName(), event.gender(), event.dateOfBirth(), event.deletedDate());
